...
# Metric name to metric_id mapping is now loaded from reference_stats_metrics.csv (metric_id column)
# to avoid maintaining metric names in two places. See _load_reference_stats_metrics().
def _load_reference_stats_metrics(pilot_data_root):
    """Load reference stats metrics from CSV file including metric_id column.

    :param pilot_data_root: str, path to pilot-data root folder.
    :return metrics: list of dict, each with section, metric, and metric_id keys.
    """
    ...
    with open(csv_path, "r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            if section and metric:
                metrics.append({"section": section, "metric": metric, "metric_id": metric_id})
    return metrics
def _build_aggregate_stats_metrics(reference_metrics):
    ...
    for ref in reference_metrics:
        ...
        if section in AGGREGATE_STATS_SKIP_SECTIONS:
            continue
        # Get metric_id from reference CSV, or generate from metric name as fallback
        ...
        result.append(entry)
    return result
def validate_site_metrics_against_reference(pilot_data_root):
    """Validate that all site stats_key-values CSVs have metric names matching the reference CSV.

    Compares the aggregatable metric names (excluding Files, Info, Pooling sections) from each
    site's stats_key-values CSV against the reference_stats_metrics.csv. Reports mismatches per site.
    Pooling is a site-specific section (FLMP only) that is not in the reference.

    :param pilot_data_root: str, path to pilot-data root folder.
    :return result: dict with keys: valid (bool), sites_checked, mismatches (list of dicts).
    """
    pilot_data_root = os.path.abspath(pilot_data_root)
    _print_section("METRIC NAME VALIDATION")
    validation_skip_sections = AGGREGATE_STATS_SKIP_SECTIONS | {"Pooling"}
    reference_metrics = _load_reference_stats_metrics(pilot_data_root)
    if not reference_metrics:
        _print_kv("Status", "ERROR - Could not load reference_stats_metrics.csv")
        _print_dash()
        return {"valid": False, "sites_checked": 0, "mismatches": [], "error": "no reference"}
    ref_metric_names = set()
    for ref in reference_metrics:
        if ref["section"] not in validation_skip_sections:
            ref_metric_names.add(ref["metric"])
    _print_kv("Reference metrics (aggregatable)", len(ref_metric_names))
    sites = _discover_site_folders(pilot_data_root)
    mismatches = []
    sites_checked = 0
    for site_code, site_folder in sites:
        ...
        if not stats_kv_path:
            continue
        sites_checked += 1
        site_metric_names = set()
        with open(stats_kv_path, "r", encoding="utf-8", newline="") as f:
            reader = csv.DictReader(f)
            for row in reader:
                section = (row.get("section") or "").strip()
                metric = (row.get("metric") or "").strip()
                if section in validation_skip_sections:
                    continue
                if metric:
                    site_metric_names.add(metric)
        in_site_not_ref = site_metric_names - ref_metric_names
        in_ref_not_site = ref_metric_names - site_metric_names
        if in_site_not_ref:
            for m in sorted(in_site_not_ref):
                mismatches.append({"site": site_code, "kind": "extra_in_site", "metric": m, "file": os.path.basename(stats_kv_path)})
    _print_kv("Sites checked", sites_checked)
    if mismatches:
        _print_kv("Status", f"FAILED - {len(mismatches)} metric name mismatch(es) found")
        print()
        print("METRIC NAME MISMATCHES (metric in site CSV but not in reference):")
        for mm in mismatches:
            print(f"  {mm['site']} [{mm['file']}]: {mm['metric']}")
        print()
        print("Fix these in the site CSVs to match reference_stats_metrics.csv, or update the reference.")
        _print_dash()
        return {"valid": False, "sites_checked": sites_checked, "mismatches": mismatches}
    _print_kv("Status", "PASSED - all site metrics match reference")
    _print_dash()
    return {"valid": True, "sites_checked": sites_checked, "mismatches": []}
...
# Google Sheets reference CSV files (relative to pilot-data folder)
...
def _load_gsheets_lookup(pilot_data_root):
    ...
    """
    filenames_csv = os.path.join(pilot_data_root, GSHEETS_FILENAMES_CSV)
    urls_csv = os.path.join(pilot_data_root, GSHEETS_URLS_CSV)
...
def run_pilot_data_pipeline(pilot_data_root, do_errant_check=True, do_zip=True, do_aggregate=True, do_markdowns=True, do_values_comparison=True):
    ...
    # Step 0: Validate site metric names against reference CSV
    validation_result = validate_site_metrics_against_reference(pilot_data_root)
    results["metrics_validation"] = validation_result
    if not validation_result.get("valid"):
        print()
        print("ABORTING PIPELINE: Metric name validation failed. Fix mismatches before proceeding.")
        _print_section("PILOT DATA PIPELINE: ABORTED")
        print()
        return results
    # Step 1: Errant check
    if do_errant_check:
...
### CSV Update Propagation
def _revert_protected_files(protected_files):
    """Check protected files for changes and revert any that were modified.

    Uses git to detect and revert changes. Files that are untracked (new) are not reverted.

    :param protected_files: list of str, file paths to protect from modification.
    :return report: list of dict, each with path, status ('reverted', 'unchanged', 'error').
    """
    import subprocess
    report = []
    for fpath in protected_files:
        ...
        if not os.path.isfile(fpath):
            report.append({"path": fpath, "status": "missing", "detail": "file does not exist"})
            continue
        try:
            result = subprocess.run(["git", "diff", "--quiet", "--", fpath], capture_output=True, text=True)
            if result.returncode != 0:
                subprocess.run(["git", "checkout", "--", fpath], capture_output=True, text=True, check=True)
                report.append({"path": fpath, "status": "reverted", "detail": "file was modified and has been reverted"})
            else:
                report.append({"path": fpath, "status": "unchanged", "detail": "file was not modified"})
        except subprocess.CalledProcessError as e:
            report.append({"path": fpath, "status": "error", "detail": str(e)})
    return report
def run_csv_update_propagation(pilot_data_root, protected_files=None):
    """Run the full pipeline to propagate CSV changes through summaries, aggregates, and zips.

    Runs the standard pipeline (errant check, zip, aggregate, markdowns, values comparison)
    followed by extended zip creation. After completion, any files listed in protected_files
    are checked for modifications and reverted if changed.

    :param pilot_data_root: str, path to pilot-data root folder.
    :param protected_files: list of str or none, file paths to protect from modification.
    :return: dict, combined results from pipeline and extended zip.
    """
    results = {}
    results["pipeline"] = run_pilot_data_pipeline(pilot_data_root, do_errant_check=True, do_zip=True, do_aggregate=True, do_markdowns=True, do_values_comparison=True)
    if results["pipeline"].get("metrics_validation", {}).get("valid") is False:
        _print_section("CSV UPDATE PROPAGATION: ABORTED (validation failed)")
        return results
    results["extended_zip"] = run_zip_pilot_data_extended(pilot_data_root)
    _print_section("CSV UPDATE PROPAGATION: COMPLETE")
    if protected_files:
        print()
        _print_section("PROTECTED FILES: REVERT CHECK")
        revert_report = _revert_protected_files(protected_files)
        results["protected_files"] = revert_report
        for entry in revert_report:
            short_path = os.path.relpath(entry["path"])
            _print_kv(f"  {short_path}", f"{entry['status'].upper()} - {entry['detail']}")
        _print_dash()
    return results
def mrun_csv_update_propagation():
    pass
#if __name__ == "__main__":
    pilot_data_root = "data/floodlamp/pilots/pilot-data"
    protected_files = [
        "data/floodlamp/pilots/pilot-data/DAVI_pilot-data/DAVI_pilot-data_referral-antigen-comparison.md",
    ]
    run_csv_update_propagation(pilot_data_root, protected_files=protected_files)

### Data Summary find and replace
def find_replace_in_pilot_data_summaries(pilot_data_root, find_str, replace_str):
    """Find and replace a string across all *_pilot-data_summary.md files.

    Scans all site folders under pilot_data_root for files matching the pattern
    {SITE}_pilot-data_summary.md and performs an exact string replacement.

    :param pilot_data_root: str, path to pilot-data root folder.
    :param find_str: str, the string to find.
    :param replace_str: str, the replacement string.
    :return result: dict with keys: files_checked (int), files_modified (int), total_replacements (int), details (list of dict).
    """
    details = []
    files_checked = 0
    files_modified = 0
    total_replacements = 0
    for entry in sorted(os.listdir(pilot_data_root)):
        site_folder = os.path.join(pilot_data_root, entry)
        if not os.path.isdir(site_folder) or not entry.endswith("_pilot-data"):
            continue
        site_code = entry.replace("_pilot-data", "")
        md_name = f"{site_code}_pilot-data_summary.md"
        md_path = os.path.join(site_folder, md_name)
        if not os.path.isfile(md_path):
            continue
        files_checked += 1
        with open(md_path, "r", encoding="utf-8") as f:
        ...
        if count > 0:
            ...
            details.append({"file": md_name, "replacements": count})
            print(f"  {md_name}: {count} replacement(s)")
        else:
            details.append({"file": md_name, "replacements": 0})
    print(f"Checked {files_checked} files, modified {files_modified}, total replacements: {total_replacements}")
    return {"files_checked": files_checked, "files_modified": files_modified, "total_replacements": total_replacements, "details": details}
def mrun_find_replace_in_pilot_data_summaries():
    pass
if __name__ == "__main__":
    pilot_data_root = "data/floodlamp/pilots/pilot-data"
    find_replace_in_pilot_data_summaries(pilot_data_root, "/edit?usp=drive_link", "")
