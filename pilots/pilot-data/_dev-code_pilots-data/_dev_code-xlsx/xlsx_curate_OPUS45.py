# ===== START OF FILE primary/xlsx_curate.py =====
...
def curate_aps_stats(input_xlsx_path, output_folder_path, site_code):
    ...
    print("\n=== QA SUMMARY ===")
    print(f"Key-values rows written: {kv_count}")
    print(f"Distinct sections found: {sections_seen}")
    print(f"Referral agreement rows written: {ref_count}")
    if referral_rows:
        print("Referral agreement parsed rows:")
...
def curate_aps_tubes(input_xlsx_path, output_folder_path, site_code):
    ...
    if 'tube_id' in df.columns:
        print(f"Unique tube_id count: {df['tube_id'].nunique()}")
    if 'pool_level_n' in df.columns:
        print(f"pool_level_n - min: {df['pool_level_n'].min()}, mean: {df['pool_level_n'].mean():.2f}, max: {df['pool_level_n'].max()}")
    if 'final_result' in df.columns:
        print(f"final_result value_counts:")
        for val, count in df['final_result'].value_counts().items():
            print(f"  {val}: {count}")
...
def curate_aps_daily_tubes_rollup(input_xlsx_path, output_folder_path, site_code):
    ...
    print("\n=== INPUT ===")
    print(f"XLSX: {input_xlsx_path}")
    print(f"Sheet: APS Tubes by Date")
    print(f"Site code: {site_code}")
    ...
    print(f"Row count: {len(df)}")
    if 'collect_date' in df.columns:
        print(f"collect_date range: {df['collect_date'].min()} to {df['collect_date'].max()}")
    
    # Sum of count columns
    for col in count_cols:
        if col in df.columns:
            total = df[col].sum()
            print(f"{col} sum: {total}")
    ...
    if 'tubes_n' in df.columns and 'sum_results_n' in df.columns:
        ...
        if len(mismatches) > 0:
            print(f"WARNING: {len(mismatches)} rows where tubes_n != sum_results_n")
...
def curate_aps_swabs_in_tubes(input_xlsx_path, output_folder_path, site_code):
    ...
    if 'tube_id' in df.columns:
        print(f"Unique tube_id count: {df['tube_id'].nunique()}")
    if 'participant_id' in df.columns:
        print(f"Unique participant_id count: {df['participant_id'].nunique()}")
    if 'app_result' in df.columns:
        print(f"app_result value_counts:")
        for val, count in df['app_result'].value_counts().items():
            print(f"  {val}: {count}")
    if 'final_result' in df.columns:
        print(f"final_result value_counts:")
        for val, count in df['final_result'].value_counts().items():
...
def curate_aps_weekly_summary(input_xlsx_path, output_folder_path, site_code):
    ...
    print(f"Row count: {len(df)}")
    if 'week_start_date' in df.columns:
        print(f"week_start_date range: {df['week_start_date'].min()} to {df['week_start_date'].max()}")
    for col in count_cols:
        if col in df.columns:
...
def curate_rfr_all_tube_results(input_xlsx_path, output_folder_path, site_code):
    ...
    print("\n=== INPUT ===")
    print(f"XLSX: {input_xlsx_path}")
    print(f"Sheet: ALL Tube Results")
    print(f"Site code: {site_code}")
    ...
    if 'tube_id' in df.columns:
        print(f"Unique tube_id count: {df['tube_id'].nunique()}")
    if 'final_assigned_run_id' in df.columns:
        print(f"Unique final_assigned_run_id count: {df['final_assigned_run_id'].nunique()}")
    if 'final_result' in df.columns:
        print(f"final_result value_counts:")
        for val, count in df['final_result'].value_counts().items():
            print(f"  {val}: {count}")
...
def curate_rtr_referral_tests(input_xlsx_path, output_folder_path, site_code):
    ...
    print("\n=== INPUT ===")
    print(f"XLSX: {input_xlsx_path}")
    print(f"Sheet: Referral Test Data")
    print(f"Site code: {site_code}")
    ...
    if 'participant_id' in df.columns:
        print(f"Unique participant_id count: {df['participant_id'].nunique()}")
    if 'referral_test_type' in df.columns:
        print(f"referral_test_type value_counts:")
        for val, count in df['referral_test_type'].value_counts().items():
            print(f"  {val}: {count}")
    if 'referral_test_result' in df.columns:
        print(f"referral_test_result value_counts:")
        for val, count in df['referral_test_result'].value_counts().items():
            print(f"  {val}: {count}")
...
def curate_rtr_referral_tests_by_person(input_xlsx_path, output_folder_path, site_code):
    ...
    print("\n=== INPUT ===")
    print(f"XLSX: {input_xlsx_path}")
    print(f"Sheet: Referral Tests by Person")
    print(f"Site code: {site_code}")
    ...
    print("\n=== QA SUMMARY ===")
    print(f"Row count: {len(df)}")
    if 'referral_overall_result' in df.columns:
        print(f"referral_overall_result value_counts:")
        for val, count in df['referral_overall_result'].value_counts().items():
            print(f"  {val}: {count}")
...
def generate_csv_manifest(output_folder_path, site_code):
    ...
    manifest_rows = [
        {
            'site_code': site_code,
            'source_workbook': f"{site_code}_APS_deID_PUB.xlsx",
            'source_tab': 'Stats',
            'output_csv': f"{site_code}_APS_stats_key-values.csv",
            'data_level': 'stats',
            'primary_key': '',
            'join_keys': '',
        ...
        },
        {
            ...
            'source_workbook': f"{site_code}_APS_deID_PUB.xlsx",
            'source_tab': 'Stats',
            'output_csv': f"{site_code}_APS_stats_referral-agreement.csv",
            'data_level': 'stats',
            'primary_key': '',
            'join_keys': '',
        ...
        },
        {
            ...
            'output_csv': f"{site_code}_APS_weekly-summary.csv",
        ...
        },
        {
            ...
            'output_csv': f"{site_code}_APS_pos-incl-cases.csv",
            'data_level': 'tube',
            'primary_key': '',
        ...
        },
        {
            ...
            'source_workbook': f"{site_code}_APS_deID_PUB.xlsx",
            'source_tab': 'postDEL',
            'output_csv': f"{site_code}_APS_swabs-in-tubes.csv",
        ...
        },
        {
            ...
            'output_csv': f"{site_code}_APS_tubes.csv",
            'data_level': 'tube',
            'primary_key': 'tube_id',
            'join_keys': 'tube_id',
        ...
        },
        {
            ...
            'output_csv': f"{site_code}_APS_daily-tubes-rollup.csv",
        ...
        {
            'site_code': site_code,
            'source_workbook': f"{site_code}_RFR_deID_PUB.xlsx",
            'source_tab': 'ALL Tube Results',
            'output_csv': f"{site_code}_RFR_all-tube-results.csv",
            'data_level': 'tube',
            'primary_key': 'tube_id',
        ...
        },
        {
            ...
            'source_workbook': f"{site_code}_RFR_deID_PUB.xlsx",
            'source_tab': 'Weekly',
            'output_csv': f"{site_code}_RFR_weekly-summary.csv",
            'data_level': 'weekly_rollup',
            'primary_key': '',
            'join_keys': 'week_start_date;week_end_date',
            'row_grain': 'one row per week',
        ...
        {
            'site_code': site_code,
            'source_workbook': f"{site_code}_RTR_deID_PUB.xlsx",
            'source_tab': 'Referral Test Data',
            'output_csv': f"{site_code}_RTR_referral-tests.csv",
            'data_level': 'referral',
        ...
        },
        {
            ...
            'output_csv': f"{site_code}_RTR_referral-tests-by-person.csv",
            'data_level': 'referral',
...
def verify_csv_against_xlsx(csv_path, xlsx_path, sheet_name, key_column, csv_to_xlsx_column_map):
    ...
    # Print summary
    if result['passed']:
        print(f"  PASSED: {matched}/{result['total_rows']} rows verified")
    else:
        print(f"  FAILED: {matched}/{result['total_rows']} rows matched, {len(result['errors'])} errors")
...
def verify_all_csvs(output_folder_path, site_code, aps_xlsx_path, rfr_xlsx_path, rtr_xlsx_path):
    ...
    """
    print("\n" + "=" * 60)
    print("VERIFICATION OF GENERATED CSVs")
    print("=" * 60)
    ...
    verifications = [
        {
            'csv': f"{site_code}_APS_tubes.csv",
            'xlsx': aps_xlsx_path,
            'sheet': 'APS Tubes',
            'key': 'tube_id',
            'map': {
        ...
        {
            'csv': f"{site_code}_APS_pos-incl-cases.csv",
            'xlsx': aps_xlsx_path,
            'sheet': 'Pos and Incl',
            'key': 'tube_id',
            'map': {
        ...
        {
            'csv': f"{site_code}_RFR_all-tube-results.csv",
            'xlsx': rfr_xlsx_path,
        ...
        {
            'csv': f"{site_code}_RTR_referral-tests.csv",
            'xlsx': rtr_xlsx_path,
            'sheet': 'Referral Test Data',
            'key': 'referral_test_id',
            'map': {
                'referral_test_id': 'REFERRAL TEST ID',
                'participant_id': 'PARTICIPANT ID',
                'referral_test_result': 'REFERRAL TEST RESULT'
            }
        },
        {
            'csv': f"{site_code}_RTR_referral-tests-by-person.csv",
            'xlsx': rtr_xlsx_path,
            'sheet': 'Referral Tests by Person',
            'key': 'participant_id',
            'map': {
                'participant_id': 'PARTICIPANT ID',
                'referral_overall_result': 'Referral Overall Result'
            }
        }
    ]
...
def process_site_xlsx_to_csv(site_code, xlsx_folder, output_folder):
    ...
    # Generate manifest
    generate_csv_manifest(output_folder, site_code)
    summary['csvs_generated'].append(os.path.join(output_folder, f"{site_code}_csv-manifest.csv"))
    
    # Run verification
    verification_results = verify_all_csvs(output_folder, site_code, aps_xlsx, rfr_xlsx, rtr_xlsx)
    ...
    print(f"CSVs generated: {len(summary['csvs_generated'])}")
...
# ===== END OF FILE primary/xlsx_curate.py =====
